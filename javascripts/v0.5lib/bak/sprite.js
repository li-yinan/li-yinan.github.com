Sprite = function(){
	...
	this.follow = function(){
		...
		_this.dest.set(sprite.anchor.x,sprite.anchor.y);
	}
	...
	this.detectCollision = function(next){
		...
		for(var i=0;i<_this.world.spriteList.length;i++){
			var dx = _this.world.spriteList[i].anchor.x;
			var dy = _this.world.spriteList[i].anchor.y;
			var dr = _this.world.spriteList[i].collisionR;
			//if is self continue
			if(_this.anchor.x==dx&&_this.anchor.y==dy){
				continue;
			}
		...
		if(collideCount == 1){
			var dx = collideSprite.anchor.x;
			var dy = collideSprite.anchor.y;
			...
			next.x = _this.anchor.x;
			next.y = _this.anchor.y;
		}
	};
	...
	this.moveTo = function(){
		...
		var sx = _this.anchor.x;
		var sy = _this.anchor.y;
		var dx = _this.dest.x;
		var dy = _this.dest.y;
		...
		_this.prevX = _this.anchor.x;
		_this.prevY = _this.anchor.y;
		_this.anchor.set(next.x,next.y);
		moveDistance = Math.sqrt((_this.anchor.x-_this.prevX)*(_this.anchor.x-_this.prevX)+(_this.anchor.y-_this.prevY)*(_this.anchor.y-_this.prevY));
		if(moveDistance<1){
			_this.setSpriteRegion(_this.direction, 0);
		}
	...
	this.getClearRegion = function(){
		var ret = {};
		ret.left = _this.anchor.x-_this.collisionR*_this.scale-1;
		ret.top = _this.anchor.y-_this.imgRegionHeight*_this.collisionR/_this.imgRegionWidth*_this.scale+_this.offset-1;
	...
	this.clear = function(){
		...
		_this.cxt.save();
		_this.cxt.translate(_this.anchor.x,_this.anchor.y);
	...
	this.draw = function(){
		...
		if(_this.drawCollisionCircle){
			...
			_this.cxt.save();
			_this.cxt.beginPath();
			_this.cxt.arc(_this.anchor.x,_this.anchor.y,_this.collisionR,0,Math.PI*2,true);
			_this.cxt.closePath();
			_this.cxt.clip();
			_this.cxt.stroke();
			_this.cxt.restore();
